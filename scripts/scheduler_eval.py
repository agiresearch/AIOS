...
def parse_global_args():
    ...
    parser.add_argument("--agent_name", type=str, required=True, choices=["MathAgent", "NarrativeAgent", "RecAgent"])
...
def main():
    ...
    agent_thread_pool = ThreadPoolExecutor(max_workers=200)

    scheduler.start()

    # construct agents
    task_inputs = load_agent_tasks(agent_name=agent_name)

    random.seed(1234)

    task_inputs = random.sample(task_inputs, k=agent_num)
